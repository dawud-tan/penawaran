...
public class LinkedMultiValueMap<K, V> implements MultiValueMap<K, V>, Serializable, Cloneable {
    ...
    public void add(K key, @Nullable V value) {
        ...
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.N) {
            values = this.targetMap.computeIfAbsent(key, k -> new LinkedList<>());
        } else {
            values = this.targetMap.get(key);
            if (values == null) {
    ...
    public void addAll(K key, List<? extends V> values) {
        ...
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.N) {
            currentValues = this.targetMap.computeIfAbsent(key, k -> new LinkedList<>());
        } else {
            currentValues = this.targetMap.get(key);
            if (currentValues == null) {
                currentValues = new LinkedList<>();
                this.targetMap.put(key, currentValues);
            }
        }
    ...
    public void setAll(Map<K, V> values) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            ...
            Iterator<K> itr = values.keySet().iterator();
            while (itr.hasNext()) {
                K key = itr.next();
    ...
    public Map<K, V> toSingleValueMap() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            ...
            Iterator<K> itr = this.targetMap.keySet().iterator();
            while (itr.hasNext()) {
                K key = itr.next();
                singleValueMap.put(key, this.targetMap.get(key).get(0));
            }
        }
    ...
    public LinkedMultiValueMap<K, V> deepCopy() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            ...
            while (itr.hasNext()) {
                K key = itr.next();
                copy.put(key, new LinkedList<>(this.targetMap.get(key)));
            }
        }
        return copy;
    }
