...
public abstract class Ed25519 {
    ...
    private static SHA512Digest createDigest() {
    ...
    private static void dom2(SHA512Digest d, byte phflag, byte[] ctx) {
        if (ctx != null) {
...
//        assert carry == 0;
    ...
    private static void implSign(SHA512Digest d, byte[] h, byte[] s, byte[] pk, int pkOff, byte[] ctx, byte phflag,
    ...
    private static boolean implVerify(byte[] sig, int sigOff, byte[] pk, int pkOff, byte[] ctx, byte phflag,
        ...
        byte[] R = Arrays.copyOfRange(sig, sigOff, sigOff + POINT_BYTES);//Decode the first half as a point R
        byte[] S = Arrays.copyOfRange(sig, sigOff + POINT_BYTES, sigOff + SIGNATURE_SIZE);//Decode the second half as an integer S, in the range 0 <= s < L
        if (!checkPointVar(R)) {
            return false;
        }
        if (!checkScalarVar(S)) {
        ...
        PointAffine pA = new PointAffine();//Decode the public key A as point A'
    ...
    public static PointExt pointCopy(PointAffine p) {
        PointExt r = new PointExt();
        X25519Field.copy(p.x, 0, r.x, 0);
        X25519Field.copy(p.y, 0, r.y, 0);
        pointExtendXY(r);
...
//            assert off == precompBase.length;
    ...
    private static void pruneScalar(byte[] n, int nOff, byte[] r) {
        ...
        r[0] &= 0xF8;////The lowest three bits of the first octet are cleared
        r[SCALAR_BYTES - 1] &= 0x7F;//the highest bit of the last octet is cleared
        r[SCALAR_BYTES - 1] |= 0x40;//the second highest bit of the last octet is set.
    }
